...
import { setCourseData, setSections } from "../../reducers/slices/courseSlice"
...
export const createSection = (token, sectionName, courseId) => {
    return async(dispatch) => {
        dispatch(setLoading(true))
        await apiConnector('POST', courseEndpoints.CREATE_SECTION_API, {
            courseId: courseId,
...
export const updateSection = (token, newName, sectionId) => {
    return async(dispatch) => {
        dispatch(setLoading(true))
        try{
            await apiConnector('PUT', courseEndpoints.UPDATE_SECTION_API, {
                sectionId: sectionId,
                sectionName: newName,
            }, {
                Authorization: `Bearer ${token}`
...
export const deleteSection = (token, sectionId) => {
    return async(dispatch) => {
        dispatch(setLoading(true))
        try{
            await apiConnector('DELETE', courseEndpoints.DELETE_SECTION_API, {
                sectionId: sectionId,
            }, {
                Authorization: `Bearer ${token}`
            })
...
export const deleteSubSection = (token, sectionId, subSectionId) => {
    return async(dispatch) => {
        dispatch(setLoading(true))
        try{
            await apiConnector('DELETE', courseEndpoints.DELETE_SUBSECTION_API, {
                sectionId: sectionId,
...
export const updateCourseStatus = (token, courseId, status) => {
    return async(dispatch) => {
        dispatch(setLoading(true))
        try{
            await apiConnector('PUT', courseEndpoints.UPDATE_COURSE_STATUS_API, {
                courseId: courseId,
                status: status,
            }, {
                Authorization: `Bearer ${token}`
...
export const addCartItem = (token, courseId) => {
    return async (dispatch) =>{
        dispatch(setLoading(true))
        try{
            await apiConnector('POST', courseEndpoints.ADD_CART_ITEM_API, {
                courseId: courseId,
            }, {
                Authorization: `Bearer ${token}`,
            })
...
export const addCourseToUser = (token, courseId) => {
    return async (dispatch) =>{
        dispatch(setLoading(true))
        try{
            await apiConnector('POST', courseEndpoints.ADD_COURSE_TO_USER, {
                courseId: courseId,
            }, {
                Authorization: `Bearer ${token}`,
            })
...
export const addCompletedLecture = (token, subSectionId, courseId) => {
    return async (dispatch) =>{
        dispatch(setLoading(true))
        try{
            await apiConnector('POST', courseEndpoints.ADD_COMPLETED_LECTURE, {
                subsectionId: subSectionId,
                courseId: courseId
            }, {
                Authorization: `Bearer ${token}`,
...
export const runCode = (code, setOutput) => {
    return async (dispatch) => {
        dispatch(setLoading(true))
        try{
            const response = await apiConnector('POST', courseEndpoints.RUN_CODE_API, {
                code: code,
            })
