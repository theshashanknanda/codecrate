import React, { useState, useEffect } from "react";
import toast from "react-hot-toast";
import { useDispatch, useSelector } from "react-redux";
import { useLocation } from "react-router-dom";
import { addCompletedLecture, runCode } from "../../services/operations/coursesApi";
import CodeEditor from "../common/CodeEditor";

const ViewCourse = () => {


  const token = useSelector((state) => state.profile.token)

  const dispatch = useDispatch()
  const { state } = useLocation();
  const { course } = state || {};

  const [currentSectionIndex, setCurrentSectionIndex] = useState(0);
  const [currentLectureIndex, setCurrentLectureIndex] = useState(0);

  const currentSection = course?.courseContent?.[currentSectionIndex];
  const currentLecture = currentSection?.subSection?.[currentLectureIndex];

//   useEffect(() => {
//     console.log("Course data:", course);
//   }, [course]);

  const handleLectureCompletion = (subsectionId) => {
    console.log("Marking as complete:", subsectionId);
    // add completed lecture to user object in db
    dispatch(addCompletedLecture(token, subsectionId, course._id))
  };

  const goToNextLecture = () => {
    handleLectureCompletion(currentLecture?._id);

    const currentSubsections = course.courseContent[currentSectionIndex].subSection;

    if (currentLectureIndex + 1 < currentSubsections.length) {
      setCurrentLectureIndex(currentLectureIndex + 1);
    } else if (currentSectionIndex + 1 < course.courseContent.length) {
      setCurrentSectionIndex(currentSectionIndex + 1);
      setCurrentLectureIndex(0);
    } else {
      toast.success("You have completed all lectures")
    }
  };

  const goToLecture = (sectionIdx, lectureIdx) => {
    setCurrentSectionIndex(sectionIdx);
    setCurrentLectureIndex(lectureIdx);
  };

  const [showSidebarMobile, setShowSidebarMobile] = useState(false);
  const [isMobile, setIsMobile] = useState(window.innerWidth < 768);
  
  useEffect(() => {
    const handleResize = () => {
      const mobile = window.innerWidth < 768;
      setIsMobile(mobile);
      setShowSidebarMobile(false); // Reset mobile state on resize
    };

    window.addEventListener('resize', handleResize);
    return () => window.removeEventListener('resize', handleResize);
  }, []);

  const toggleSidebar = () => {
    setShowSidebarMobile(!showSidebarMobile);
  };

  // Always show sidebar on desktop
  const showSidebar = !isMobile || showSidebarMobile;

  return (
    <div className="flex flex-col md:flex-row max-h-[100vh - 100px] bg-[#0F0F0F] relative text-left overflow-hidden m-0 p-0">
      {/* Mobile Toggle Button */}
      <div className="md:hidden bg-[#1A1A1A] p-4 sticky top-0 text-left">
        <button
          onClick={toggleSidebar}
          className="text-gray-300 z-[-20] hover:text-white focus:outline-none w-full text-left"
        >
          {showSidebarMobile ? 'Hide' : 'Show'} Course Content
        </button>
      </div>

      {/* Sidebar */}
      {showSidebar && (
        <div className="md:block w-full md:w-[300px] bg-[#1A1A1A] p-4 overflow-y-auto h-[calc(100vh-80px] flex-shrink-0">
          <h2 className="text-lg sm:text-xl font-semibold mb-4 text-left">Course Content</h2>
          {course?.courseContent?.map((section, sectionIdx) => (
            <div key={sectionIdx} className="mb-6">
              <p className="text-[#CBAB09] font-medium text-sm sm:text-base">{section.sectionName}</p>
              <div className="mt-2 space-y-1">
                {section.subSection?.map((sub, subIdx) => {
                  const isActive =
                    currentSectionIndex === sectionIdx &&
                    currentLectureIndex === subIdx;
                  return (
                    <div
                      key={sub._id}
                      className={`cursor-pointer px-3 py-2 rounded-lg text-sm transition-colors ${
                        isActive
                          ? "bg-[#CBAB09] text-white"
                          : "text-gray-300 hover:bg-[#2C2C2C]"
                      }`}
                      onClick={() => goToLecture(sectionIdx, subIdx)}
                    >
                      {sub.title}
                    </div>
                  );
                })}
              </div>
            </div>
          ))}
        </div>
      )}
      
      {/* Main Content */}
      <div className="flex-1 flex flex-col overflow-scroll"> {/* Parent container to manage flex context */}
        {/* Video Section */}
        <div className="w-full bg-black flex-shrink-0">
          {currentLecture?.videoUrl ? (
            <div className="relative w-full" style={{ paddingTop: '56.25%' }}>
              <video
                src={currentLecture.videoUrl}
                controls
                className="absolute top-0 left-0 w-full h-full object-contain"
                controlsList="nodownload"
                playsInline
              />
            </div>
          ) : (
            <div className="w-full h-96 flex items-center justify-center text-gray-400">
              No Video Available
            </div>
          )}
        </div>

        {/* Lecture Details */}
        <div className="p-4 md:p-6 text-left flex-shrink-0">
          <h3 className="text-xl sm:text-2xl font-semibold">{currentLecture?.title}</h3>
          <p className="text-gray-400 text-sm sm:text-base mt-2">
            {currentLecture?.description}
          </p>
          <div className="mt-4">
            <button
              onClick={goToNextLecture}
              className="px-4 sm:px-6 py-2 bg-[#CBAB09] hover:bg-[#b39609] rounded-lg text-black font-medium text-sm sm:text-base transition-colors"
            >
              Next Lecture
            </button>
          </div>
        </div>
        
        {/* Code Editor Section - This will grow and scroll internally */}
        <div className="flex-1 w-full">
          <CodeEditor />
        </div>
      </div>
    </div>
  );
};

export default ViewCourse;
